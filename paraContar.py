...
while(True):
 ...
 df = pandas.DataFrame(columns=["SERVIDOR","CAMARA",'JUNIO','JULIO','AGOSTO','SEPTIEMBRE','OCTUBRE','NOVIEMBRE','DICIEMBRE','ENERO','OTROS','VIDEOS'])

 if __name__ == '__main__':

  for i in range(1, 38):
    
    if i not in [2,9,10,18,33,34,36]:
 ...
 df.sort_values(by=['DICIEMBRE'], ascending=[False], inplace=True)

 engine = create_engine('postgresql://postgres:admin@localhost:5432/sap_db')

 df[['SERVIDOR','CAMARA','DICIEMBRE','OTROS','VIDEOS','MULTAS']].to_sql('conteos_conteo', engine, if_exists='replace')
 #df[['SERVIDOR', ' CAMARA', 'NOVIEMBRE','OTROS', 'VIDEOS']].to_html('home/sistemas/Django/personas_django/sap/webapp/templates/bienvenido.html')
